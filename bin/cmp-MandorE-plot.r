...
for (dat in data) {
	...
	for (column in columns[2:length(columns)]) {
		if (all(is.na(dat[column]))) next
		else {
			mins.maxs[[i]][[column]] <- matrix(NA, nrow=2, ncol=2)
			mins.maxs[[i]][[column]][1,1] = min(dat[column.1], na.rm=TRUE)
			mins.maxs[[i]][[column]][1,2] = max(dat[column.1], na.rm=TRUE)
			mins.maxs[[i]][[column]][2,1] = min(dat[column], na.rm=TRUE)
			mins.maxs[[i]][[column]][2,2] = max(dat[column], na.rm=TRUE)
		}
	}
	i <- i+1
}
...
for (column in columns[2:length(columns)]) {
  ...
  for (df in refData) {
    ...
    if (!is.element(column,colnames(df)) || all(is.na(df[column]))) skip <- TRUE
      ...
      mmxy[[i]][[column]] <- matrix(NA, nrow=2, ncol=2)
	  mmxy[[i]][[column]][1,1] = min(min.1, min(df[column.1], na.rm=TRUE), na.rm=TRUE)
	  mmxy[[i]][[column]][1,2] = max(max.1, max(df[column.1], na.rm=T), na.rm=T)
	  mmxy[[i]][[column]][2,1] = min(min.2, min(df[column], na.rm=TRUE), na.rm=TRUE)
	  mmxy[[i]][[column]][2,2] = max(max.2, max(df[column], na.rm=TRUE), na.rm=TRUE)
	  i <- i+1
    }
  }
  ...
  for (df in data.ma) {
    ...
    if (plot.data && !zeroed) {
      ...
      if (use.frames || ndx == 1) {
        ...
        if (use.frames) {
          mainTitle <- paste(titles[[ndx]],column,sep="-")
		...
		min.x <- mins.maxs[[ndx]][[column]][1,1]
		max.x <- mins.maxs[[ndx]][[column]][1,2]
		min.y <- mins.maxs[[ndx]][[column]][2,1]
		max.y <- mins.maxs[[ndx]][[column]][2,2]
        ## Plot the raw data
      ...
      if (use.frames || ndx == 1) {
		...
		min.y <- mins.maxs[[ndx]][[column]][2,1]
		max.y <- mins.maxs[[ndx]][[column]][2,2]
        mainTitle <- ifelse(use.frames, paste(titles[[ndx]],"-",column,", maw = ",maws[[ndx]], sep=""), paste(titles[[ndx]],column,sep="-"))
